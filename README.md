...
- **Капча після 3 невдалих спроб** (Cloudflare Turnstile). Ліміти вище
  зупиняють перебір одного акаунта, але не «один злитий пароль до тисячі
  акаунтів» — від цього рятує лише виклик, який людина проходить за дві
  секунди, а скрипт не проходить взагалі. Показується на вході в адмінку, на
  вході в кабінет і у формі відновлення пароля — і **лише** після кількох
  промахів, а не з першої спроби.
  Щоб увімкнути: dash.cloudflare.com → Turnstile → Add site (безкоштовно, без
  картки), і додати обидва ключі у Vercel. Доки ключів немає — віджет не
  рендериться, скрипт Cloudflare не вантажиться, CSP лишається без сторонніх
  доменів.
